...
- 页面主体包含商品、评价、订单明细等可增长内容时，统一使用 `Scroll` 或 `List`，并通过 `layoutWeight(1)` 占满顶部固定区域以下的剩余空间。
- 购物车栏、提交栏等底部操作区作为滚动组件的同级子组件放在外层 `Column` 中；滚动内容增加底部 padding，避免最后一项被遮挡。
- 独立详情组件不要在父级 `Column` 中使用 `height('100%')`，应由父级和组件根节点共同使用 `layoutWeight(1)`。
- 评价按钮始终渲染并根据订单状态显示“去评价”或“评价”，不可评价时点击后给出完成订单提示。
- 地址选择流程使用 `AppStorage` 传递选择模式：结算页进入地址列表，选择已有地址后设置 `currentAddress` 并返回；定位确认必须先调用后端地址接口取得有效 ID，再持久化当前地址。
- 登录成功和恢复登录态时同步当前账号的地址列表，优先使用有效的当前地址或默认地址；数据归一化要保留合法地址数组，不能无条件清空。
- 任何按钮点击都要有反馈：校验失败、重复点击、请求中、成功、失败分别提示用户；不要使用禁用按钮或直接 `return` 制造“点击无反应”。订单提交前优先校验店铺起送价并提示差额，接口失败优先展示服务端返回消息。
