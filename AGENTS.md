...
- **底部 Tab 结构（2026-09 收敛）**：4 个标签 `首页 / AI助手 / 购物车 / 我的`（`Index.ets` 的 `Tabs` + `tabBuilder(title, 0|1|2|3)`）。原「发现」Tab 已删除，附近推荐并入首页（`HomePage` 内 `recommendedStores` + `RecommendedStoreCardHost`，需要定位与登录）；原「订单」Tab 已删除，改为独立路由页 `pages/OrderListPage`（从「我的」页的订单入口与资产区「订单」进入，入口统一先写 `AppStorage['orderFilterStatus']`）。**购物车 Tab 与独立路由页 `pages/CartPage` 共用同一份实现**：`CartPage` 新增 `@Prop showNavBar`，Tab 内传 `false`（Tab 页没有上一级，不显示返回键），路由页用默认 `true`。**新增/调整 Tab 时必须同步 `Index.ets` 的各处 TabContent、`tabBuilder` 的 targetIndex、以及 `.onChange` 里刷新「我的」的 index 判断**（现为 `index === 3`）。
