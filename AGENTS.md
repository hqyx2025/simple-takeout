...
- **底部 Tab 结构（2026-09 收敛）**：只剩 3 个标签 `首页 / AI助手 / 我的`（`Index.ets` 的 `Tabs` + `tabBuilder(title, 0|1|2)`）。原「发现」Tab 已删除，附近推荐并入首页（`HomePage` 内 `recommendedStores` + `RecommendedStoreCardHost`，需要定位与登录）；原「订单」Tab 已删除，改为独立路由页 `pages/OrderListPage`（从「我的」页的订单入口与资产区「订单」进入，入口统一先写 `AppStorage['orderFilterStatus']`）。**新增/调整 Tab 时必须同步 `Index.ets` 的三处 TabContent、`tabBuilder` 的 targetIndex、以及 `.onChange` 里刷新「我的」的 index 判断**（现为 `index === 2`）。购物车从来不是 Tab，走 `pages/CartPage` 独立路由页。
