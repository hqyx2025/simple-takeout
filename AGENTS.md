...
- **店铺详情页分类芯片的空转根因在种子数据**：芯片走的是美团式「店内分类」（`categories.type='MERCHANT'` + `goods.merchant_category_id`），链路早已就绪但种子数据从未建过 MERCHANT 分类、1200 个商品的 `merchant_category_id` 全为 0 → 芯片只剩「全部 + 该店唯一平台分类」，两者商品列表完全相同，点分类「看起来没反应」。由 `DataSeeder.ensureStoreMerchantCategories()` 幂等补齐：**判断条件必须是「该商户还有 `merchant_category_id=0` 的商品」，不能只看「有没有分类」**（商户可能自己建过分类但商品没挂上去，只看分类存在与否会让这类店铺继续空转），且只更新 `=0` 的行以免覆盖商户已有分组。
