...
- **店铺照片链路（商户上传 → 用户端展示）**：stores.image 存上传接口返回的相对地址（/uploads/yyyyMMdd/xxx.png，列宽 255）。写入侧只有两处：StoreService.createStore(..., image)（新建）与 StorePatch.image（编辑，null=不修改）。读取侧用户端统一用 merchantImageSource(store.image)：有照片显示照片，没有则**保留首字母色块**（店铺列表/卡片不能让所有店都显示同一张默认图，否则顾客分不清店），只有店铺详情页顶部大图才回退 DEFAULT_MERCHANT_IMAGE。商户端新建/编辑两个表单都有「店铺照片」上传项，商户店铺列表也显示缩略图。配送半径已打通：`MerchantStoreRequest/Patch` 带 `deliveryRadius`，两个表单都有「配送半径（米）」输入，编辑默认回传当前值（旧代码不传该字段 → 服务端按 0 落库，保存店铺即静默清零）。**半径语义：商户填了按商户的，没填（0）按默认 2 公里`Store.DEFAULT_DELIVERY_RADIUS_METERS`，不再表示"不限"**；判定收口在 `Store.canDeliver`，三处消费：`OrderService.createOrder` 超范围直接拒绝下单、`StoreService.recommendedStores` 过滤附近推荐、`StoreView.deliverable` 给前端列表过滤。
