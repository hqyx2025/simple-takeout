...
| 骑手（RIDER） | 需手动开通 | **`DataSeeder` 不创建骑手账号**，见下方说明 |

> **骑手账号需要手动开通**：种子数据只创建用户/商户/管理员三类账号，
> 因此全新数据库上骑手端无法直接登录。任选一个已有账号（密码 `123456`）改为骑手角色即可，
> 骑手档案会在首次访问骑手端时自动建档：
>
> ```sql
> -- 示例：把副用户 13900139000 改成骑手
> UPDATE users SET role = 3 WHERE phone = '13900139000';
> ```
>
> 之后用 `loginType=RIDER` 登录即可（登录页选择「骑手端」）。
